...
def compareCells (file1,file2):
    ...
    for inputLigne, outputLigne in zip(file1,file2):
        ...
        for inputCell, outputCell in zip(inputLigne,outputLigne):
            ...
            if inputCell !="":
                multipleCells = inputCell.split(':'), outputCell.split(':')
                if len(multipleCells[0]) != len(multipleCells[1]) :
                    print('error on ligne %s at ligne %s and column %s'%(inputLigne, (inputData.index(inputLigne)+2),(inputLigne.index(inputCell)+1)))
                    print( 'The number of subcells is different')
                    sys.exit(107)
                for indexSubCell, (inputSubCell, outputSubCell) in enumerate(zip(*multipleCells)) :
                    #Convert cell value in float and check if the values weren't close enough
                    if abs(float(outputSubCell)-float(inputSubCell)) > sys.float_info.epsilon:
                        print('error on ligne %s at ligne %s and column %s'%(inputLigne, (inputData.index(inputLigne)+2),(inputLigne.index(inputCell)+1)))
                        print( 'the input cell %s is different from output cell %s at subcell index %s'%(inputCell,outputCell, indexSubCell))
                        sys.exit(107)
