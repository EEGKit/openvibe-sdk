...
int urImportScenarioFromFileTest(int argc, char* argv[])
{
...

	{
		...
		context->getPluginManager().addPluginsFromFiles(OpenViBE::Directories::getLibDir() + "/libopenvibe-plugins-base-tools*so");
		...
		context->getPluginManager().addPluginsFromFiles(OpenViBE::Directories::getLibDir() + "/libopenvibe-plugins-base-tools*dylib");
		#endif
