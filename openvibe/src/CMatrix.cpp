...
bool CMatrix::isDescriptionEqual(const CMatrix& m, const bool checkLabels) const
{
	...
	if (checkLabels)
	{
		...
		{
			for (size_t j = 0; j < getDimensionSize(i); ++j) { if (getDimensionLabel(i, j) != m.getDimensionLabel(i, j)) { return false; } }
		}
	}
	return true;
}
...
void CMatrix::resize(const size_t dim1, const size_t dim2)
{
	...
	{
		if (getDimensionCount() != 2 || getDimensionSize(0) != dim1 || getDimensionSize(1) != dim2) // check if size don't change
		{
			setDimensionCount(2);
			setDimensionSize(0, dim1);
	...
	reset();	// Initialise buffer with 0 always (if size don't change too
}
//--------------------------------------------------------------------------------
...
bool CMatrix::toTextFile(const std::string& filename) const
{
	...
	{
		file << "\t" << "[ ";
		for (const auto& label : dimension) { file << "\"" << label << "\" "; }
		file << "]" << std::endl;
	}
	file << "]" << std::endl;
	...
	file << "[" << std::endl;
	file << toString("\t", "[ ", " ", "]");
	if (m_dimSizes->size() == 1) { file << std::endl; }		// If we have 1 Dimension, it's a simple row without break line
	file << "]" << std::endl;
...
bool CMatrix::fromTextFile(const std::string& filename)
{
	...
	// resize matrix (don't use resize functions to avoïd delete actual labels
	...
	return fromString(ss.str(), "\t", "[ ", " ", "]");			// use stringstream to parse the matrix
}
//--------------------------------------------------------------------------------

//--------------------------------------------------------------------------------
bool CMatrix::fromString(const std::string& in, const std::string& before, const std::string& start, const std::string& sep, const std::string& end) const
{
...
//--------------------------------------------------------------------------------
std::string CMatrix::toString(const std::string& before, const std::string& start, const std::string& sep, const std::string& end) const
{
	std::stringstream ss;
	ss.precision(10);
