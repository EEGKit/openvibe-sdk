...
namespace Common
{
	namespace Resampler
	{
		...
		public:
			...
			{
				if (iFractionalDelayFilterSampleCount < 6 || 30 < iFractionalDelayFilterSampleCount) { return false; }
			...
			{
				if (iMaxInputSampleCount < 8 || 2048 < iMaxInputSampleCount) { return false; }
			...
			{
				if (f64TransitionBandInPercent < r8b::CDSPFIRFilter::getLPMinTransBand() || r8b::CDSPFIRFilter::getLPMaxTransBand() < f64TransitionBandInPercent) { return false; }
			...
			{
				if (f64StopBandAttenuation < r8b::CDSPFIRFilter::getLPMinAtten() || r8b::CDSPFIRFilter::getLPMaxAtten() < f64StopBandAttenuation) { return false; }
		...
		private:
			...
			size_t resample_channel_wise(const ICallback& rCallback, const TFloat* pInputSample, size_t ui32InputSampleCount)
			{
				...
				for (uint32_t j = 0; j < m_ui32ChannelCount; j++)
				{
					...
					{
						l_vInputBuffer[k] = double(pInputSample[k * m_ui32ChannelCount + j]);
					}

					l_iCount = 0;
					double* l_pResamplerOutputBuffer;
					l_iCount = m_vResampler[j]->process(&l_vInputBuffer[0], int(ui32InputSampleCount), l_pResamplerOutputBuffer);
					...
					{
						l_vOutputBuffer[k * m_ui32ChannelCount + j] = TFloat(l_pResamplerOutputBuffer[k]);
					}
				}
			...
			size_t resample_sample_wise(const ICallback& rCallback, const TFloat* pInputSample, size_t ui32InputSampleCount)
			{
				...
				for (size_t j = 0; j < m_ui32ChannelCount; j++)
				{
					...
					{
						l_vInputBuffer[k] = double(pInputSample[j * ui32InputSampleCount + k]);
					}
