...
boolean OpenViBEToolkit::initialize(const IKernelContext& rKernelContext)
{
	...
	l_rTypeManager.registerEnumerationEntry(OV_TypeId_Stimulation, "OVTK_StimulationId_Unknown", OVTK_StimulationId_Unknown);
	...
	if (rKernelContext.getConfigurationManager().expandAsBoolean("${Kernel_ExtendedStimulationLabels}", false))
	{
		// Create all labels up to 0xFF
		for (int i = 0x20; i <= 0xFF; i++)
		{
			char label[255];
			sprintf(label, "OVTK_StimulationId_Label_%02X", i);
			l_rTypeManager.registerEnumerationEntry(OV_TypeId_Stimulation, label, OVTK_StimulationId_Label(i));
		}
	}
	...
	l_rTypeManager.registerEnumerationEntry(OV_TypeId_Stimulation, "OVTK_StimulationId_ThresholdPassed_Positive", OVTK_StimulationId_ThresholdPassed_Positive);
	l_rTypeManager.registerEnumerationEntry(OV_TypeId_Stimulation, "OVTK_StimulationId_ThresholdPassed_Negative", OVTK_StimulationId_ThresholdPassed_Negative);

	l_rTypeManager.registerEnumerationEntry(OV_TypeId_Stimulation, "OVTK_StimulationId_NoArtifact", OVTK_StimulationId_NoArtifact);
	l_rTypeManager.registerEnumerationEntry(OV_TypeId_Stimulation, "OVTK_StimulationId_Artifact", OVTK_StimulationId_Artifact);
