...
CPluginObjectDescEnumAlgorithmSnapshotGenerator::~CPluginObjectDescEnumAlgorithmSnapshotGenerator(void)
{
	...
	FS::Files::openOFStream(l_oAlgorithmsFile, (m_sDocTemplateDirectory+"/Doc_Algorithms.dox").c_str());
	
	if(!l_oAlgorithmsFile.good())
	{
	...
	for(auto itCategories : m_vCategories)
	{
...
boolean CPluginObjectDescEnumAlgorithmSnapshotGenerator::callback(const IPluginObjectDesc& rPluginObjectDesc)
{
	...
	::GdkPixbuf* l_pPixBuf=gdk_pixbuf_get_from_drawable(
		...
		0, 0,
		xSize+32, ySize+32);
	if(! gdk_pixbuf_save(l_pPixBuf, (l_sFilename+".png").c_str(), "png", NULL, NULL))
	{
		m_rKernelContext.getLogManager() << LogLevel_Error << "Error while saving snapshot to [" << (l_sFilename+".png").c_str() << "]\n";
	...
	FS::Files::openOFStream(l_oFile, (l_sFilename+".dox").toASCIIString());
