	...
	,m_sDocTemplateDirectory(sDocTemplateDirectory)
{
	...
	gdk_color_set(m_vColors[Color_BoxBackgroundDeprecated], 65535, 50000, 32767);
	gdk_color_set(m_vColors[Color_BoxBackgroundMetabox],    58343, 65535, 62343);
	gdk_color_set(m_vColors[Color_BoxBackgroundMensia],     65535, 65535, 65535);
	...
	gdk_color_set(m_vColors[Color_BoxBorderMensia],         10000, 45535, 35535);
...
CPluginObjectDescEnumBoxAlgorithmSnapshotGenerator::~CPluginObjectDescEnumBoxAlgorithmSnapshotGenerator(void)
{
	...
	FS::Files::openOFStream(l_oBoxAlgorithmsFile, (m_sDocTemplateDirectory+"/Doc_BoxAlgorithms.dox").c_str());

	if(!l_oBoxAlgorithmsFile.good())
	{
		m_rKernelContext.getLogManager() << LogLevel_Error << "Error while trying to open file [" << (m_sDocTemplateDirectory+"/Doc_BoxAlgorithms.dox").c_str() << "]\n";
		return;
	}
	...
	for(auto& l_rCategories : m_vCategories)
	{
		string l_sCategory=l_rCategories.first;
		string l_sName=l_rCategories.second;
	...
	l_oBoxAlgorithmsFile << "\n\n - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -\n";
	l_oBoxAlgorithmsFile << "* The following boxes are deprecated, they are hidden in Studio and will be removed soon or later, so you should consider not using them:\n";
	
	for(auto& l_rDeprecatedCategories : m_vDeprecatedBoxesCategories)
	{
		string l_sCategory=l_rDeprecatedCategories.first;
		string l_sName=l_rDeprecatedCategories.second;
...
boolean CPluginObjectDescEnumBoxAlgorithmSnapshotGenerator::callback(const IPluginObjectDesc& rPluginObjectDesc)
{
	...
	if (rPluginObjectDesc.getCreatedClass() == OVP_ClassId_BoxAlgorithm_Metabox)
	{
		...
		if (!m_pScenario->addBox(l_oBoxIdentifier, static_cast<const OpenViBE::Plugins::IBoxAlgorithmDesc&>(rPluginObjectDesc), OV_UndefinedIdentifier))
		{
	...
	else if(!m_pScenario->addBox(l_oBoxIdentifier, rPluginObjectDesc.getCreatedClassIdentifier(), OV_UndefinedIdentifier))
	{
		m_rKernelContext.getLogManager() << LogLevel_Warning << "Skipped [" << CString(l_sFilename.c_str()) << "] (could not create corresponding box)\n";
		return true;
	}
	...
	if(m_rKernelContext.getPluginManager().isPluginObjectFlaggedAsMensia  (rBox.getAlgorithmClassIdentifier())
			|| l_bIsMetabox)
	{
		gdk_draw_pixbuf(m_pWidget->window, l_pDrawGC, m_pMensiaLogoPixbuf, 5, 5, xStart, yStart, 80, (ySize < 50)?ySize:50, GDK_RGB_DITHER_NONE, 0, 0);
		l_iBorderColor = Color_BoxBorderMensia;
	}

	gdk_gc_set_rgb_fg_color(l_pDrawGC, &m_vColors[l_iBorderColor]);
	gdk_draw_rounded_rectangle(
		m_pWidget->window,
		l_pDrawGC,
		FALSE,
	...
	// draw a second border around metaboxes
	if (l_bIsMetabox)
	{
		gdk_gc_set_rgb_fg_color(l_pDrawGC, &m_vColors[l_iBorderColor]);
		...
		gdk_draw_rounded_rectangle(
					...
					l_pDrawGC,
					FALSE,
					xStart - 3, yStart - 3, xSize + 6, ySize + 6);
	}
	...
	::GdkPixbuf* l_pPixBuf=gdk_pixbuf_get_from_drawable(
		...
		0, 0,
		xSize+32, ySize+32);
	if(!gdk_pixbuf_save(l_pPixBuf, (m_sSnapshotDirectory+"/Doc_"+l_sFilename+".png").c_str(), "png", NULL, NULL))
	{
		m_rKernelContext.getLogManager() << LogLevel_Error << "Error while saving snapshot to [" << (m_sSnapshotDirectory+"/Doc_"+l_sFilename+".png").c_str() << "]\n";
	...
	FS::Files::openOFStream(l_oFileSkeleton, (m_sDocTemplateDirectory+"/Doc_"+l_sFilename+".dox-skeleton").c_str());
	FS::Files::openOFStream(l_oFilePart, (m_sDocTemplateDirectory+"/Doc_"+l_sFilename+".dox-part-skeleton").c_str());

	if(!l_oFileSkeleton.good() || !l_oFilePart.good())
	{
		m_rKernelContext.getLogManager() << LogLevel_Error << "Error while trying to open files [" 
			<< (m_sDocTemplateDirectory+"/Doc_"+l_sFilename+".dox-skeleton").c_str() 
			<< (m_sDocTemplateDirectory+"/Doc_"+l_sFilename+".dox-part-skeleton").c_str()
			<< "]\n";
		return false;
	}
	...
	l_oFileSkeleton
		...
		<< " * \\image latex Doc_" << l_sFilename.c_str() << ".png\n"
		<< " *\n";
...
// "\\ref Doc_Types_" << transform(l_sTypeName.toASCIIString()).c_str()
	...
	// m_vCategories is used to generate the list of boxes. Documentation for deprecated boxes
	...
	{
		m_vDeprecatedBoxesCategories.push_back(pair < string, string >(rPluginObjectDesc.getCategory().toASCIIString(), rPluginObjectDesc.getName().toASCIIString()));
	}
