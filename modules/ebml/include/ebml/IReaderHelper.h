...
namespace EBML
{
	...
	public:
		 ...
		 */
		virtual uint64_t getUIntegerFromChildData(const void* pBuffer, uint64_t size) = 0;
		/**
		 * \brief Gets a signed integer from the given buffer
		 */
		virtual int64_t getSIntegerFromChildData(const void* pBuffer, uint64_t size) = 0;
		/**
		 * \brief Gets a float from the given buffer
		 */
		virtual double getFloatFromChildData(const void* pBuffer, uint64_t size) = 0;
		// virtual ??? getFloat80FromChildData(const void* pBuffer, const uint64_t size)=0;
		// virtual ??? getDateFromChildData(const void* pBuffer, const uint64_t size)=0;
		/**
		 ...
		 */
		virtual const char* getASCIIStringFromChildData(const void* pBuffer, uint64_t size) = 0;
		// virtual ??? getUTF8StringFromChildData(const void* pBuffer, const uint64_t size)=0;
		//@}
