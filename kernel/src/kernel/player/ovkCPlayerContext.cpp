	...
	namespace Kernel
	{
		namespace
		{
			...
			public:
				...
				__BridgeBindFunc1__(getKernelContext().getAlgorithmManager(), CIdentifier, createAlgorithm, , const Plugins::IAlgorithmDesc&, rAlgorithmDesc)
			...
			public:
				...
				__BridgeBindFunc2__(getKernelContext().getConfigurationManager(), boolean, registerKeywordParser, ,const CString&, rKeyword, const IConfigurationKeywordExpandCallback&, rCallback);
				__BridgeBindFunc1__(getKernelContext().getConfigurationManager(), boolean, unregisterKeywordParser, ,const CString&, rKeyword);
				__BridgeBindFunc1__(getKernelContext().getConfigurationManager(), boolean, unregisterKeywordParser, ,const IConfigurationKeywordExpandCallback&, rCallback);

				__BridgeBindFunc1__(getKernelContext().getConfigurationManager(), CString, expand, const, const CString&, rExpression);
				__BridgeBindFunc2__(getKernelContext().getConfigurationManager(), CString, expandOnlyKeyword, const, const CString&, rKeyword, const CString&, rExpression);
...
{
	return m_pSimulatedBox->getScheduler().getCurrentLateness();
}

float64 CPlayerContext::getCurrentCPUUsage(void) const
{
	return m_pSimulatedBox->getScheduler().getCPUUsage();
}

float64 CPlayerContext::getCurrentFastForwardMaximumFactor(void) const
{
	return m_pSimulatedBox->getScheduler().getFastForwardMaximumFactor();
}
