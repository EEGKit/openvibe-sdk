...
bool CBoxAlgorithmClassifierTrainer::initialize()
{
	...
	OV_ERROR_UNLESS_KRF(configFilename != CString(""), "Invalid empty configuration filename", OpenViBE::Kernel::ErrorType::BadSetting);
	...
	if (strategyClassID == OV_UndefinedIdentifier)
	{
		...
		OV_ERROR_UNLESS_KRF(classifierAlgorithmID != OV_UndefinedIdentifier,
							"Unable to instantiate classifier for class [" << classifierAlgorithmID.str() << "]", OpenViBE::Kernel::ErrorType::BadConfig);
	...
	{
		m_sampleDecoder.push_back(new OpenViBEToolkit::TFeatureVectorDecoder<CBoxAlgorithmClassifierTrainer>());
	...
	for (size_t i = 1; i < nInput; ++i)
	{
		for (size_t j = 0; j < boxContext.getInputChunkCount(i); ++j)
		{
			...
			if (m_sampleDecoder[i - 1]->isBufferReceived())
			{
				...
				OpenViBEToolkit::Tools::Matrix::copy(*sample.sampleMatrix, *sampleMatrix);
