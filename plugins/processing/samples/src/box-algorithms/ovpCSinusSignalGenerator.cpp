...
boolean CSinusSignalGenerator::initialize(void)
{
	...
	int32 l_i32ChannelCount=atoi(l_sChannelCount);
	int32 l_i32SamplingFrequency=atoi(l_sSamplingFrequency);
	int32 l_i32GeneratedEpochSampleCount=atoi(l_sGeneratedEpochSampleCount);
	m_bHeaderSent=false;

	if(l_i32ChannelCount <= 0)
	{
		this->getLogManager() << LogLevel_Error << "Channel count can not be negative. At least 1 channel required. Check box settings.\n";
		return false;
	}
	m_ui32ChannelCount = static_cast<uint32>(l_i32ChannelCount);

	if(l_i32SamplingFrequency <= 0)
	{
		this->getLogManager() << LogLevel_Error << "Sampling rate can not be negative. Check box settings.\n";
		return false;
	}
	m_ui32SamplingFrequency = static_cast<uint32>(l_i32SamplingFrequency);

	if(l_i32GeneratedEpochSampleCount <= 0)
	{
		this->getLogManager() << LogLevel_Error << "Epoch sample count can not be negative. An epoch must have at least 1 sample. Check box settings.\n";
		return false;
	}
	m_ui32GeneratedEpochSampleCount = static_cast<uint32>(l_i32GeneratedEpochSampleCount);
