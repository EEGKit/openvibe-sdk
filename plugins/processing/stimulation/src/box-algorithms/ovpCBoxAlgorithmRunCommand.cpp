...
boolean CBoxAlgorithmRunCommand::initialize(void)
{
	...
	{
		CString l_sCommandString = FSettingValueAutoCast(*this->getBoxAlgorithmContext(), i+1);
		m_vCommand[(uint64)FSettingValueAutoCast(*this->getBoxAlgorithmContext(), i)].push_back(l_sCommandString);
	}
