...
void CAlgorithmXMLScenarioImporter::openChild(const char* sName, const char** sAttributeName, const char** sAttributeValue, XML::uint64 ui64AttributeCount)
{
	...
	else if(l_sTop=="Entry"               && m_ui32Status==Status_ParsingScenario) { m_ui32Status=Status_ParsingMetadataEntry;     m_pContext->processStart(OVTK_Algorithm_ScenarioExporter_NodeId_MetadataEntry); }
...
void CAlgorithmXMLScenarioImporter::closeChild(void)
{
	...
	else if(l_sTop=="Entry"               && m_ui32Status==Status_ParsingMetadataEntry)     { m_ui32Status=Status_ParsingScenario; m_pContext->processStop(); }
...
bool CAlgorithmXMLScenarioImporter::validateXML(const unsigned char* xmlBuffer, unsigned long xmlBufferSize)
{
	...
	if(this->validateXMLAgainstSchema((OpenViBE::Directories::getDataDir() + "/kernel/openvibe-scenario-v2.xsd"), xmlBuffer, xmlBufferSize)) {
		return true;
	}
