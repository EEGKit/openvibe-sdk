...
boolean CBoxAlgorithmCSVFileWriter::initialize(void)
{
	...
	m_pFile = FS::Files::open(l_sFilename.toASCIIString(), "wb");
	if(!m_pFile)
	{
		this->getLogManager() << LogLevel_Error << "Could not open file [" << l_sFilename << "]\n";
		return false;
	}
	...
	{
		this->getLogManager() << LogLevel_Trace << "Compression flag not used yet, the file will be flagged uncompressed and stored as is\n";
	}
...
boolean CBoxAlgorithmCSVFileWriter::process_streamedMatrix(void)
{
	...
	for(uint32 i=0; i<l_rDynamicBoxContext.getInputChunkCount(0); i++)
	{
		...
		if(m_pStreamDecoder->isOutputTriggerActive(OVP_GD_Algorithm_StreamedMatrixStreamDecoder_OutputTriggerId_ReceivedHeader))
		{
			...
			{
				::fprintf(m_pFile, "Chunk Start Time(s)%sChunk End Time(s)",m_sSeparator.toASCIIString());
			}
			else
			{
		...
		if(m_pStreamDecoder->isOutputTriggerActive(OVP_GD_Algorithm_StreamedMatrixStreamDecoder_OutputTriggerId_ReceivedBuffer))
		{
			for(uint32 s=0; s<m_pMatrix->getDimensionSize(1); s++)
			{
				if(m_oTypeIdentifier == OV_TypeId_StreamedMatrix)
				{
					::fprintf(m_pFile, "%f%s%f", 
						(l_ui64StartTime >> 16) / 65536.,
						m_sSeparator.toASCIIString(),
						(l_ui64EndTime >> 16) / 65536.);
				}
				if(m_oTypeIdentifier == OV_TypeId_Signal)
				{
					::fprintf(m_pFile, "%f", ((l_ui64StartTime + ((s*(l_ui64EndTime - l_ui64StartTime)) / op_pMatrix->getDimensionSize(1))) >> 16) / 65536.);
				}
				if(m_oTypeIdentifier == OV_TypeId_Spectrum)
				{
					::fprintf(m_pFile, "%f", (l_ui64EndTime >> 16) / 65536.);
				}
