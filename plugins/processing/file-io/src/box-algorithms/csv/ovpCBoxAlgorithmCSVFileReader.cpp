...
boolean CBoxAlgorithmCSVFileReader::initialize(void)
{
	...
	std::string l_sLine = std::string(l_pLine);

	// If the delimiter is a whitespace, we must delete special chars, remove leading, trailing and extra whitespaces, before split the line.
	if (m_sSeparator == " ")
	{
		l_sLine.erase(l_sLine.find_last_not_of("\t\n\r\f\v") + 1);
		l_sLine = std::regex_replace(l_sLine, std::regex("^ +| +$|( ) +"), "$1");
	}

	m_vHeaderFile = split(l_sLine, m_sSeparator);
	...
	else if(this->getTypeManager().isDerivedFromStream(m_oTypeIdentifier,OV_TypeId_Signal))
	{
		...
		std::vector<std::string> l_vParsed = split(l_sLine, m_sSeparator);
		...
		m_ui64SamplingRate = std::stoll(l_vParsed[m_ui32NbColumn - 1]);
...
boolean CBoxAlgorithmCSVFileReader::process(void)
{
	...
	if(m_vLastLineSplit.size() == 0)
	{
		...
		while(!feof(m_pFile) && l_ui32NbSamples<m_ui32SamplesPerBuffer && fgets(l_pLine, m_ui32bufferLen, m_pFile) != NULL )
		{
			...
			m_vLastLineSplit = split(l_sLine , m_sSeparator);
	...
	l_bStimulationSend = (m_vLastLineSplit.size() != 0) && (std::stod(m_vLastLineSplit[0]) < l_f64currentTime);
	...
	if(this->getTypeManager().isDerivedFromStream(m_oTypeIdentifier,OV_TypeId_Stimulations)
			...
			|| this->getTypeManager().isDerivedFromStream(m_oTypeIdentifier,OV_TypeId_Spectrum))
	{
		while (m_vLastLineSplit.size() != 0 && std::stod(m_vLastLineSplit[0])<l_f64currentTime)
		{
			m_vDataMatrix.push_back(m_vLastLineSplit);
...
void CBoxAlgorithmCSVFileReader::convertVectorDataToMatrix(IMatrix* matrix)
{
		...
		for(uint32 i=0;i<m_vDataMatrix.size();i++)
		{
			...
			for(uint32 j = 0; j < m_ui32NbColumn - 1; j++)
			{
				if (!m_vDataMatrix[i][j + 1].empty())
				{
					float64 l_f64InterpretedValue;
					bool l_bIsStringToDoubleSuccess = true;

					try
					{
						l_f64InterpretedValue = std::stod(m_vDataMatrix[i][j + 1]);
					}
					catch (const std::invalid_argument &e)
					{
						getLogManager() << LogLevel_Warning << "Failed to interpret data [" << m_vDataMatrix[i][j + 1].c_str() << "] to double: " << e.what() << "\n";
						l_bIsStringToDoubleSuccess = false;
					}

					if (l_bIsStringToDoubleSuccess)
					{
						matrix->getBuffer()[j * m_vDataMatrix.size() + i] = l_f64InterpretedValue;
